...
class GemmaAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        attn_weights = torch.matmul(query_states, key_states.swapaxes(2, 3)) / math.sqrt(self.head_dim)
...
class GemmaModel(GemmaPreTrainedModel):
    ...
    def _update_causal_mask(self, attention_mask, input_tensor):
        ...
        if hasattr(self, "causal_mask"):  # we use the current dtype to avoid any overflows
            causal_mask = (
                self.causal_mask[None, None, :, :].tile((batch_size, 1, 1, 1)).to(dtype) * torch.finfo(dtype).min
            )
        else:
            mask = torch.full(
